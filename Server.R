...
shinyServer(function(input, output, session) {
  ...
  output$info_timeline<- renderText({
    if (is.null(input$tl_package))
      paste("<center>Please select package(s)") 
    if (!is.null(input$tl_package)) {
      obs <- paste("<center>Observation period:",stats_Luminescence$date[1],"to",stats_Luminescence$date[length(stats_Luminescence$date)])
      stats <- character(length(input$tl_package))
      for (i in 1:length(input$tl_package)) {
        stats[i] <- paste("Total downloads:", paste0("'",input$tl_package[i],"'")," =", nrow(get(paste0("stats_", input$tl_package[i]))))
      }
      paste(obs, "<br>", paste(stats, collapse = "<br>"))
    }
  })
  ...
  output$plot_timeline<- renderGvis({
    input$ab_plot
    isolate({
      if (is.null(input$tl_package)) return(NULL)
  ...
  output$plot_pie<- renderGvis({
    df<- arrange(as.data.frame(table(subset(get(paste0("stats_",input$pie_package)), select = input$pie_vars))), Freq, Var1)
    df<- setorder(as.data.table(df), -Freq)
    gvisPieChart(df, options=list(width=val$width, height=val$height))
    
  })
  
  output$plot_hist<- renderGvis({
    df<- arrange(as.data.frame(table(subset(get(paste0("stats_",input$hist_package)), select = input$hist_vars))), Var1, Freq)
    df<- setorder(as.data.table(df), -Freq)
  ...
  output$plot_globe<- renderGlobe({
    ...
    args <- c(col, list(lat = val2$lat, long = val2$lon, value = val2$Freq * input$globe_scale/100, atmosphere = TRUE))
